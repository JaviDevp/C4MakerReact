...
export const ExternalSoftwareSystemNode = ({ id, data, selected }) => {
	const [formValues, handleInputChange] = useForm({
		...
		description:
			data.description !== undefined ? data.description : 'Descripion',
	});
	...
	return (
		<div
			className={`${
				selected ? 'border-dashed border-2 border-gray-600 rounded-md p-1' : ''
			}`}>
			<div
				className={`bg-zinc-400 border border-black flex flex-col p-2 rounded-sm `}>
				<Handle
					type='source'
				...
				<div className='mb-2'>
					<textarea
						...
						onChange={handleInputChange}
						value={description}
						className='bg-transparent text-center focus:outline-none text-gray-200'></textarea>
				</div>
			</div>
		</div>
	);
